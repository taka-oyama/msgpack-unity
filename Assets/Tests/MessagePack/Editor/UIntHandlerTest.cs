...
namespace MessagePack.Tests
{
	public class UIntHandlerTest : TestBase
	{
		...
		{
			uint value = 0;
			byte[] data = Pack<uint>(value);
			uint result = Unpack<uint>(data);
		...
		{
			uint value = Unpack<uint>(ReadFile("Ints/PositiveFixIntMin"));
		...
		{
			uint value = Unpack<uint>(ReadFile("Ints/PositiveFixIntMax"));
		...
		{
			uint value = Unpack<uint>(ReadFile("Ints/UInt8Min"));
		...
		{
			uint value = Unpack<uint>(ReadFile("Ints/UInt8Max"));
		...
		{
			uint value = Unpack<uint>(ReadFile("Ints/UInt16Min"));
		...
		{
			uint value = Unpack<uint>(ReadFile("Ints/UInt16Max"));
		...
		{
			uint value = Unpack<uint>(ReadFile("Ints/UInt32Min"));
		...
		{
			uint value = Unpack<uint>(ReadFile("Ints/UInt32Max"));
		...
		{
			Assert.Throws<FormatException>(() => Unpack<uint>(ReadFile("Ints/UInt64Min")));
		}

		[Test]
		public void UnpackUInt64MaxAsUInt()
		{
			Assert.Throws<FormatException>(() => Unpack<uint>(ReadFile("Ints/UInt64Max")));
		}
